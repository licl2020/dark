...
(function(_p, _jq){
	...
	_p.add = function(o){
	    ...
	    if(this.isExisted(id)){
	        ...
	        if((typeof max == 'number') && (this.getTotal() < max)){
	        	...
	        	_jq.showConfirm('当前活动的页签数过多，请关闭后再操作！');
	        }
	    }
	};
	...
	// 获取当前页add时传入的对象
	_p.getConfig = function(win){
		var li=this.getSelf(win);
    	if(li){
			return li[0].config;
		}
    	return null;
	...
	_p.create = function(o){
		...
		head[0].config=o;
		if(this.singlePage){
			_this.headContainer.empty().append(head);
		}else{
		...
		if(this.singlePage){
			_this.bodyContainer.empty().append(body);
		}else{
			_this.bodyContainer.append(body);
		}
	...
	_p.getHtml = function(type, o){
	    ...
	    switch(type){
			case 'head':
	            ...
	            html.push('<h4 id="' + o.id + '-title" title="' + o.display + '">'+'<i class="iconfont icon-dingwei"></i>当前位置：' + o.display + '</h4>');
	            html.push(btnClose);
	            html.push('</li>');
	            break;
	        case 'body':
	            var url = core.modifyUrl(o.url, 'random', Math.random());
	...
	_p.showBody = function(){
	    ...
	    if(len == 2){
	        ...
	        if(body.length > 0){
	            ...
	            body.css('display', 'block').siblings().remove();
