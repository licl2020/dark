...
$(function(){
    ...
    // 身份证出生日期校验
    jQuery.validator.addMethod("IDEquaDate", function(value, element) { 
        value=parseInt(value);    
        var date=$(element).parents('.cardnum-box').prev().prev('.borth-box').find('.Wdate').val()||$(element).parents('.cardnum-box').prev('.borth-box').find('.Wdate').val();
        if(date!=''&&typeof date!='undefined'&&date!=null){        	
        	if($(element).hasClass('isIdCardNo')){
        		date=date.replace('-','').replace('-','');
        		var cardId=$(element).val().substring(6,14);       		
        	} 
        var result=(cardId==date);
        }
        return result||null;       
   }, "身份证出生日期不符");  
    //姓名
    jQuery.validator.addMethod("zy", function(value, element) { 
        return this.optional(element) || /^[\u4e00-\u9fa5_a-zA-Z]+$/.test(value);     
   }, "只能输入英文汉字"); 
    ...
    //数字字母类型
    jQuery.validator.addMethod("digOrnum", function(value, element) { 
        value=parseInt(value);      
