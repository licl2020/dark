var loadUi = {
	initialize: function () {
		this.grid = $('#grid');//列表
		this.btnAdd = $('#btnAdd');
		this.btnZtAll = $('#btnZtAll');
		this.btnYYAll = $('#btnYYAll');
		this.btnDel = $('.btnDel');
	...
	initEvent: function () {
		...
		this.btnEdit.on('click',function(){
			...
			$.showDialog({
				...
				title: '修改用户',
				url: cuurrent_host+'/job/update?groupName='+groupName+"&jobName="+jobName,
				width: 660,
				height: 360
			});
		});
		this.btnZtAll.on('click',function(){
			$.showConfirm({
				icon: 'confirm',
				content: '确认暂停所有的任务吗？',
				cancel: true,
				ok: function () {
				     $.ajax({
			               url: cuurrent_host+"/job/updateallstate",
			               type: 'POST',
			               dataType: 'json',
			               data : {
			   				'state' : 2
		...
		this.btnYYAll.on('click',function(){
		...
		this.btnReset.on('click',function(){
			...
			$.showConfirm({
				...
				ok: function () {
				     $.ajax({
			               url: cuurrent_host+"/job/resetpassword",
			               type: 'POST',
			               dataType: 'json',
			               data : {
			   				'id' : id
		...
		this.updatestate.on('click',function(){
			...
			$.showConfirm({
				...
				ok: function () {
				     $.ajax({
			               url: cuurrent_host+"/job/updatestate",
			               type: 'POST',
			               dataType: 'json',
			               data : {
			            	'groupName' : groupName,
	...
	gridOption: {
			...
			buttons: [
				{
					...
					onClick: function (data) {
						$.showDialog({
							...
							tabId: tabs.getSelfId(window),
							title: '修改用户',
							url: cuurrent_host+'/job/update.action',
							width: 660,
							height: 360
						});
					}
		...
		url: cuurrent_host+'/job/list.action',
		dataType: 'json',
		usePage: true,
		pageCross: true,
		type : "POST",  
