        ...
        var g = {
        	...
        	initSelected: function(p){
		        ...
		        // 从文本框data-selected属性中获取勾选项
		        	...
		        	if(p.text.val() == ''){
		        		p.text.removeAttr('data-selected');
		        	}
		        ...
		        if(core.isArray(p.selected) && p.selected.length > 0){
					...
					p.text.attr('data-selected',p.selected);
		        }else{
		            // 移除所有复选框的勾选状态
		            allCheckboxs.removeAttr('checked');
		        }
        	...
        	addProp: function(p){
        		var _g = this;
        		if(core.isFunction(o.beforeShow)){
					
					o.beforeShow(this,p,p.list);
        		...
        		.on('click.checkbox', ':checkbox', function(e){
					// 回调勾选时事件处理函数
					if(core.isFunction(o.beforeOnSelect)){
						if(o.beforeOnSelect(this,p) == false){
							return false;	
						}
					}
                    // 赋值
        	...
        	setValue: function(p){
		        ...
		        // 文本框赋值
		       	p.text.val(display.join(',')).attr('data-selected', value.join(',')).focus();
		       	
		       	// 回调勾选时事件处理函数
		        if(core.isFunction(o.onSelect)){
		            o.onSelect(result,p.text);
		        }
        	},
        	// 定位选项容器
        ...
        return this.each(function(){
			...
			if(o.options){
				 for(var i= 0 ;i < o.options.length; i++){
					 if(o.selected){
						for(var x=0 ;x<o.selected.length;x++){
							o.selected[x] == o.options[i].value && p.array.push(o.options[i].display);	 
						}
						_this.attr('data-selected', o.selected);
					 }
				}
			}
			
			_this.val(p.array.join(","));
        	// 缓存文本框
			if(_this.is('div')){
            ...
            p.box.addClass('ui-multiselecter').off('click.multiselecter').on('click.multiselecter', function(e){
            	// 收起组件
				var text=_this.val().split(","),arr=[];
				for(var i= 0 ;i < text.length; i++){
					if(o.options){
						for(var x= 0 ;x < o.options.length; x++){
							if(o.options[x].display == text[i]){
								arr.push(o.options[x].value);
								break
							}
						}
					}
