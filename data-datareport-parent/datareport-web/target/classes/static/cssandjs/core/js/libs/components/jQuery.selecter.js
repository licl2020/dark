...
(function($){
	...
	var g = {
			...
			$.ajax({
				...
				success: function(result){
					...
					for(var i = 0, len = result.length; i < len; i++){
						var option = result[i], value, display, selected, flag = core.isPlainObject(option);
						value = flag ? option.value : option;
						display = flag ? option.display : option;
						selected = flag ? option.selected : option;
		...
		getSkin: function(target, o){
			var tg = $(target), html = [], id = '', className = '', style = '', disabled = '', readonly = '', value = '',sheng='',xian='',city='';
			...
			sheng=tg.find('option:selected').attr('data-sheng');
			xian=tg.find('option:selected').attr('data-xian');
			city=tg.find('option:selected').attr('data-city');

			
			html.push('<div class="ui-text ui-selecter' + className + (disabled?" ui-text-disabled":"")+'"' + style + '>');
			html.push('<input type="text"' + id + ' class="ui-text-text"' + disabled + readonly + ' value="' +value+ '" data-city="'+city+'" data-sheng="'+sheng+'" data-xian="'+xian+'" placeholder="'+($(target).attr('placeholder') || "")+'"/>');
			html.push('<i class="ui-text-icon iconfont">&#xe689;</i>');
		...
		dropdown: function(select, target, o){
			...
			oOptions.each(function(){
				...
				nOptionsHtml.push('<li title="' + text + '"' + (disabled ? 'class="disabled"  disabled="disabled"' : '') + '><label>' + text + '</label></li>');
			});
			...
			nOptions.click(function(e){
				...
				select.find(':text').val(value).focus();
