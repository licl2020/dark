...
(function ($) {
	var // Save a reference to some core methods
			...
			isPlainObject: function (obj) {
				...
				try {
					if (
						...
						!_hasOwnProperty.call(obj.constructor.prototype, "isPrototypeOf")
					) {
						return false;
					}
				} catch (e) {
			...
			clone: function (obj) {
				...
				for (var key in obj) {
					if (obj.hasOwnProperty(key)) {
						n[key] = this.isPlainObject(obj[key])
							? this.clone(obj[key])
							: obj[key];
					}
				}
		...
		_rules = [
			...
			{
				name: "telephone",
				tips: "电话号码格式不正确,如：0571-88016679",
				rule: /^((((\(\d{2,3}\))|(\d{3}\-))?(\(0\d{2,3}\)|0\d{2,3}-)?[1-9]\d{6,7}(\-\d{1,4})?)|(((400-)|(800-))\d{3}-\d{4}?))$/
			},
			{ name: "qq", tips: "QQ号码格式不正确", rule: /^[1-9][0-9]{4,}$/ },
			{
				name: "idCard",
				tips: "身份证格式不正确",
				rule: /^[1-9]([0-9]{14}|[0-9]{17})$/
			},
			{ name: "zipCode", tips: "邮政编码格式不正确", rule: /^[1-9]\d{5}$/ },
			{
				name: "url",
				tips: "URL地址格式不正确,如：http://www.baidu.com",
				rule: /^http:\/\/[A-Za-z0-9]+\.[A-Za-z0-9]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/
			},
			{
				name: "ip4",
				tips: "IP地址格式不正确",
				rule: /^(25[0-5]|2[0-4]\d|[0-1]\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|[0-1]\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|[0-1]\d{2}|[1-9]?\d)\.(25[0-5]|2[0-4]\d|[0-1]\d{2}|[1-9]?\d)$/
			},
			{
				name: "rar",
				tips: "不是有效的压缩文件",
				rule: /(.*)\.(rar|zip|7zip|tgz)$/
			},
			{
				name: "picture",
				tips: "图片格式不正确",
				rule: /(.*)\.(jpg|bmp|gif|ico|pcx|jpeg|tif|png|raw|tga)$/
			},
			...
			{
				name: "account",
				tips: "用户名必须由数字、字母或下划线组成",
				rule: /^\w+$/
			},
			{
				name: "specialAccount",
				tips: "用户名必须由中英文、数字或下划线组成",
				rule: /^[a-zA-Z0-9_\u4e00-\u9fa5]+$/
			},
		...
		_vQuery = {
			...
			verifyAllElems: function (selector) {
				var _this = this,
					...
					flag = true;
				for (
					var index = 0, total = _this.globalCaches.length;
					index < total;
					index++
				) {
					var json = this.globalCaches[index],
						elem = $(json.selector, form);
					if (elem.length > 0) {
						// Text & password & textarea
						if (
							elem.is(":text") ||
							...
							elem.each(function () {
								...
								for (var i = 0, len = rules.length; i < len; i++) {
									var flagN = _this.verifyText(this, rules[i]);
									flag = flagN && flag;
									if (!flagN) {
										break;
									}
						...
						// Radio & checkbox
							...
							var rules = json.rules;
							for (var i = 0, len = rules.length; i < len; i++) {
								var flagN = _this.verifyCheck(elem, localCache[i]);
								flag = flagN && flag;
								if (!flagN) {
									break;
								}
			...
			verifyText: function (elem, json) {
				var flag = true,
					errorStyle = this.errorStyle.text;
				...
				if (flag) {
					...
					if (elem.parent().find(".fml-error").attr("class")) {
						elem.parent().find(".fml-error").remove();
					}
				} else {
					// Add title & class
					elem.parent().attr("title", json.tips).addClass(errorStyle);
					if (elem.parent().find(".fml-error").attr("class")){elem.parent().find('.fml-error').remove()}
					elem.parent().append('<div class="fml-error fml-red">' + json.tips + "</div>");
					elem.parent().find('.fml-error').show()
				}
			...
			verifyCheck: function (elem, json) {
				var flag = true,
					errorStyle = this.errorStyle.box;
				...
				$(selector).each(function () {
					...
					if (elem.is(":text") || elem.is(":password") || elem.is("textarea")) {
						elem
							.parent()
							.removeAttr("title")
							.removeClass(_this.errorStyle.text);
					}
					// Radio & checkbox
					else if (elem.is(":radio") || elem.is("checkbox")) {
