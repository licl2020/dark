...
Core4j.browser = {
    ...
    mozilla: /mozilla/.test(_tmp_ua_lc_core4j) && !/(compatible|webkit)/.test(_tmp_ua_lc_core4j)
};
...
Core4j.packageRegister = function(packagepath) {
    ...
    for (var i = 0; i < packageArray_length; i++) {
        ...
        str_eval += "if (typeof(" + str_package + ") == 'undefined') " + str_package + " = new Object();";
    }
    if (str_eval != "") {
        eval(str_eval);
    }
...
Core4j.Domhelper.setElAttributeValue = function(elobj, attributeName, value) {
    if (value != null) {
        if (attributeName == "class" || attributeName == "className" || attributeName == "classStyle") {
            ...
            if (attributeName == "style" && Core4j.browser.msie) {
